...
import { Lightbulb, Layers, Zap, Brain, Hammer } from "lucide-react";
        ...
        <div className="space-y-8 animate-in fade-in duration-700 pb-20">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                <button className={cn(
                    ...
                    !category ? "bg-accent text-base border-accent" : "bg-secondary text-silk/40 border-white/5"
                )}>
                    すべて
                </button>
                {categories.map((cat) => (
                    <button
                        ...
                        className={cn(
                            "px-4 py-2 rounded-full text-xs font-bold transition-all whitespace-nowrap border",
                            category === cat ? "bg-accent text-base border-accent" : "bg-secondary text-silk/40 border-white/5"
                        )}
                    >
                        {cat}
                    </button>
            ...
            <div className="grid gap-4">
                {tips.length > 0 ? (
                    tips.map((tip) => {
                        ...
                        return (
                            <div key={tip.id} className="premium-card glass space-y-3 group border-white/5">
                                ...
                                <h3 className="text-silk font-bold group-hover:text-accent transition-colors">{tip.title}</h3>
